function isUtilsContentPayloadShape(value: unknown): value is Record<string, unknown> {
...
function isToolCallBlock(block: unknown): block is ToolCall {
  return (
    ...
    isUtilsContentPayloadShape((block as { arguments?: unknown }).arguments)
  );
}
...
export function summarizeContentForTranscript(
  ...
  return content
    .map((b) => {
      ...
      if (isUtilsContentPayloadShape(b) && (b as { type?: unknown }).type === "tool_use") {
      ...
      if (isUtilsContentPayloadShape(b) && (b as { type?: unknown }).type === "tool_result") {
        const raw =
      ...
      if (isUtilsContentPayloadShape(b) && typeof (b as { type?: unknown }).type === "string") {
