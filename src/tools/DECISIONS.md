...
- **Restore identity:** `chart_absent` checkpoints capture the stable Office.js chart id at creation; restore locates the chart by id (rename-proof, never deletes an unrelated chart that reused the name) and falls back to unique-name matching only when ids are unavailable. Chart restores return the post-restore identity so inverse (rollback) snapshots are addressed where the chart actually is after a rename-restore.
