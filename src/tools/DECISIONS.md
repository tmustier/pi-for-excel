...
- **Never-saved workbooks:** a workbook with no path-derived identity gets backups scoped to a random token stored in the document's add-in settings (`doc_instance:<uuid>`, see `src/workbook/recovery-scope.ts`). The token is minted only when the first backup is captured (the workbook is already dirty by then), never by list/restore/polling, and never on hosts that cannot store document settings (WPS, browser). It is not an alias: once the workbook has a path-derived identity, token-scoped backups are no longer visible, because Save As / Save a Copy duplicate the token. Manual full-workbook backups (`/backup`) stay canonical-identity-only for the same reason. Host limitation (Mac Excel, observed 2026-09-10): after a mid-session Save As, `Office.context.document.url` stays empty until the file is closed and reopened, even across an add-in reload, so the token scope stays in use until then; the save boundary still clears backups on dirty → saved, but backups captured after that first save and before a reopen become invisible once the path identity appears.
