...
async function applySheetNameState(
  context: Excel.RequestContext,
  targetState: RecoverySheetNameState,
...
async function applySheetVisibilityState(
  context: Excel.RequestContext,
  targetState: RecoverySheetVisibilityState,
): Promise<RecoveryModifyStructureState> {
  const sheet = await loadSheetById(context, targetState.sheetId);
  if (!sheet) {
    throw new Error("Sheet referenced by structure checkpoint no longer exists.");
...
async function applySheetAbsentState(
  context: Excel.RequestContext,
  targetState: RecoverySheetAbsentState,
): Promise<RecoveryModifyStructureState> {
...
async function applySheetPresentState(
  context: Excel.RequestContext,
...
async function applyRowsState(
  context: Excel.RequestContext,
  targetState: RecoveryRowsAbsentState | RecoveryRowsPresentState,
): Promise<RecoveryModifyStructureState> {
...
async function applyColumnsState(
  context: Excel.RequestContext,
  targetState: RecoveryColumnsAbsentState | RecoveryColumnsPresentState,
): Promise<RecoveryModifyStructureState> {
  const position = normalizePositiveInteger(targetState.position);
  const count = normalizePositiveInteger(targetState.count);
  if (position === null || count === null) {
