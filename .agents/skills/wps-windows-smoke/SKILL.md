...
- Ordinary WPS personal-account login does **not** unlock this block in the current Windows ARM WPS `12.1.0.26200` harness. The block reproduces after logged-in trust install with a minimal alert-only add-in and with a clean current official ET template built from `wpsjs@2.2.3` (including `functions.json`). Public WPS community threads report version-sensitive add-in regressions and one workaround of using 32-bit WPS instead of 64-bit; treat Windows ARM / `win-arm64ec` host behavior as suspect and prefer testing a supported x86/x64/32-bit or WPS 365 managed build before more Pi-side debugging.
