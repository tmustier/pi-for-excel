...
- **GPT-5.5 is now the preferred default when OpenAI is available**
  - Refreshed the Pi stack to `0.70.0` and pulled the latest model registry through `@earendil-works/pi-ai`.
...
- Pi packages (`@earendil-works/pi-ai`, `@earendil-works/pi-agent-core`, `@earendil-works/pi-web-ui`) remain in lockstep at `0.70.0`.
