...
At release verification time, Luna was fully discoverable and configurable through the bundled canonical registry. Native Pi `0.80.6` completed `openai-codex/gpt-5.6-luna` successfully through its Node `auto`/WebSocket transport, and the OpenAI API/OpenRouter routes also completed successfully. The ChatGPT SSE route used by the Excel WebView reproducibly returned upstream `Model not found gpt-5.6-luna`; the same Excel path completed Sol and Terra.
...
- `npm run build`
- `npm run validate`
- Browser taskpane visual check: GPT-5.6 selector ordering plus separate **Extra high** and **Max** reasoning choices
