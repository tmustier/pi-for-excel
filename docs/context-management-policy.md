...
- Runtime capability refresh in `src/taskpane/init.ts` now fingerprints tool metadata and only calls `agent.setTools(...)` when the fingerprint changes (avoids no-op tool churn during refresh passes).
